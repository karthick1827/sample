...
Load this file when acl-prd is invoked headless (no interactive user). Follow it for the whole run.
...
**Update.** Apply the change, log it via `uv run {project-root}/_acl/scripts/memlog.py append --workspace {doc_workspace} --type change --text "<change + rationale>"`, and surface any conflict-with-prior-decision in `conflicts_with_prior_decisions[]` in the JSON status. Halt `blocked` if intent is ambiguous.
