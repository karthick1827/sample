...
AGENTS = {
    "acl-agent-analyst": {"name": "Mary", "icon": "📊", "title": "Analyst"},
    "acl-agent-pm": {"name": "John", "icon": "📋", "title": "PM"},
}


class TestAlias(unittest.TestCase):
    def test_strips_known_prefixes(self):
        self.assertEqual(rp._alias("acl-agent-analyst"), "analyst")
        self.assertEqual(rp._alias("acl-foo"), "foo")
...
class TestBuildCollective(unittest.TestCase):
    def test_installed_agents_indexed_by_code_alias_and_name(self):
        ...
        self.assertEqual(set(col), {"acl-agent-analyst", "acl-agent-pm"})
        self.assertEqual(idx["analyst"], "acl-agent-analyst")      # alias
        self.assertEqual(idx["mary"], "acl-agent-analyst")         # name (ci)
        self.assertEqual(idx["acl-agent-pm"], "acl-agent-pm")     # full code
        self.assertEqual(col["acl-agent-analyst"]["source"], "installed")
    ...
    def test_custom_overrides_installed_by_alias(self):
        ...
        self.assertEqual(col["acl-agent-analyst"]["source"], "custom")
        self.assertEqual(col["acl-agent-analyst"]["name"], "Mary-Custom")
...
class TestResolveMembers(unittest.TestCase):
    ...
    def test_resolves_in_listed_order_and_flags_unknowns(self):
        ...
        self.assertEqual([m["code"] for m in resolved], ["morpheus", "acl-agent-analyst"])
...
class TestInstalledCodesIsDefaultRoom(unittest.TestCase):
    ...
    def test_pure_custom_excluded_override_kept_in_default_room(self):
        ...
        self.assertEqual(installed, ["acl-agent-analyst", "acl-agent-pm"])
        default_room = [col[c]["code"] for c in installed]
        self.assertEqual(default_room, ["acl-agent-analyst", "acl-agent-pm"])
