...
The forge cross-examines witnesses: the installed ACL agents, plus any
custom personas and party groups the user has authored for `acl-party-mode`.
...
What it returns (JSON, stdout):
  * agents   — the installed ACL roster: the default room, always present.
...
`acl-party-mode`'s resolved customization when that skill is found beside
...
def load_agents(project_root: Path):
    """Installed ACL agents as {code: entry}. (dict, resolved_ok).
    ...
    """
    script = project_root / "_acl" / "scripts" / "resolve_config.py"
...
def find_party_skill(project_root: Path, skill_root: Path):
    """Locate the installed acl-party-mode skill dir, or None.
    ...
    candidates = [
        ...
        project_root / "_acl" / "skills" / PARTY_SKILL,
    ]
    for c in candidates:
...
def load_party_workflow(project_root: Path, party_skill: Path):
    """Merged [workflow] table for acl-party-mode (base + user overrides)."""
    resolver = project_root / "_acl" / "scripts" / "resolve_customization.py"
...
def load_party_overrides(project_root: Path):
    ...
    """
    custom = project_root / "_acl" / "custom"
...
def _alias(code: str) -> str:
    """Short alias for an installed agent code: acl-agent-analyst -> analyst."""
    for prefix in ("acl-agent-", "acl-"):
...
def main():
    ...
    ap.add_argument("--skill", required=True, help="Path to the acl-forge-idea skill dir")
    args = ap.parse_args()
