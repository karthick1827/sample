...
# ///
"""render.py — acl-quick-dev template renderer.

Resolves compile-time {{.variable}} placeholders from ACL's central config,
...
files to {project-root}/_acl/render/acl-quick-dev/.

Config: four-layer merge of _acl/config.toml + config.user.toml +
custom/config.toml + custom/config.user.toml (post-#2285 installs).
Keys surface from [core] and [modules.acl]. Missing or unparseable
...
_acl/custom/acl-quick-dev.toml + .user.toml (same structural rules as
...
def find_project_root():
    """Walk up from cwd until a _acl/ directory is found. On failure, print a
    HALT instruction to stdout and exit non-zero."""
    current = os.path.abspath(os.getcwd())
    while True:
        candidate = os.path.join(current, "_acl")
        if os.path.isdir(candidate):
            return current
        parent = os.path.dirname(current)
        if parent == current:
            print(
                f"HALT and report to the user: no _acl/ directory found walking up from {os.getcwd()}"
            )
            sys.exit(1)
        current = parent
    ...
    if not os.path.isfile(path):
        if required:
            print(
                ...
                "ensure this is a post-#2285 ACL install"
            )
            sys.exit(1)
        return {}
    try:
...
def _deep_merge(base, override):
    ...
    flat scalars out of [core] and [modules.acl])."""
...
def resolve_workflow(root, skill_dir, skill_name):
    ...
    custom_dir = posixpath.join(root, "_acl", "custom")
...
def load_central_config(root):
    """Four-layer merge of _acl/config.toml and its peers (highest priority
    last). HALTs if the base _acl/config.toml is missing or unparseable."""
    acl_dir = posixpath.join(root, "_acl")
    base_team = load_toml(posixpath.join(acl_dir, "config.toml"), required=True)
    base_user = load_toml(posixpath.join(acl_dir, "config.user.toml"))
    custom_team = load_toml(posixpath.join(acl_dir, "custom", "config.toml"))
    custom_user = load_toml(posixpath.join(acl_dir, "custom", "config.user.toml"))
...
def flatten_central_config(merged):
    """Lift scalar keys from [core] and [modules.acl] into a single namespace.
    ...
    for section in (merged.get("core"), modules.get("acl")):
...
def _scalar_str(value):
    ...
    ACL config conventions), None as empty, everything else via str()."""
    if value is None:
        return ""
    if isinstance(value, bool):
...
def main():
    ...
    if not implementation_artifacts:
        print(
            ...
            "(expected under [core] or [modules.acl] in _acl/config.toml)"
        )
        sys.exit(1)
    ...
    out_dir = posixpath.join(root, "_acl", "render", skill_name)
